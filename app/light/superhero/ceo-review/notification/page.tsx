...
const APPROVAL_URL = "/light/superhero/ceo-review";
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
        ...
        <div className="w-full max-w-2xl">
          ...
          <div className="rounded-2xl border border-[#e5e7eb] bg-[#f9fafb] overflow-hidden shadow-xl">
            <div className="border-b border-[#e5e7eb] bg-white px-6 py-4">
              ...
              <div className="flex items-center gap-2 text-[#9ca3af] text-xs mb-2">
                <span>To:</span>
                <span className="text-[#111827]">{CEO_NAME} &lt;{CEO_EMAIL}&gt;</span>
              </div>
              <div className="flex items-center gap-2 text-[#9ca3af] text-xs">
                <span>Subject:</span>
                <span className="text-[#111827] font-medium">Undisclosed risks: 10-K and ERM deck ready for your approval</span>
              </div>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="rounded-xl border border-[#e5e7eb] bg-white p-4">
                <p className="text-[10px] font-medium uppercase tracking-wider text-[#9ca3af] mb-3">Risks requiring disclosure</p>
                <div className="space-y-2">
                  {RISKS.map((risk) => (
                    <div key={risk.label} className="flex items-center justify-between">
                      <span className="text-sm text-[#111827]">{risk.label}</span>
                      <span className={cn(
                        "rounded px-1.5 py-0.5 text-[10px] font-semibold",
                        risk.severity === "Critical" ? "bg-[#da3633]/20 text-[#dc2626]" : "bg-[#d29922]/20 text-[#ca8a04]"
                      )}>
                        {risk.severity}
                      </span>
                    </div>
              ...
              {/* Documents */}
              <div className="rounded-xl border border-[#e5e7eb] bg-white p-4">
                <p className="text-[10px] font-medium uppercase tracking-wider text-[#9ca3af] mb-3">Documents for approval</p>
                <div className="space-y-2">
                  <div>
                    <p className="text-sm font-semibold text-[#111827]">10-K Risk Factor Draft — FY2025</p>
                    <p className="text-xs text-[#6b7280]">3 risk factors updated with team feedback incorporated</p>
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-[#111827]">ERM Board Deck — Q1 2026</p>
                    <p className="text-xs text-[#6b7280]">4 slides covering top risks, trends, and changes from prior cycle</p>
                  </div>
                </div>
              </div>
      ...
      <StakeholderFooter label={`Continue as ${CEO_NAME} (CEO) to advance the workflow`}>
        <Link href="/light/superhero/gc-review-feedback" className="text-sm text-[#6b7280] hover:text-[#374151]">
          ← Back to GC review
        </Link>
