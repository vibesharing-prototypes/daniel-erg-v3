            ...
            <div className="mt-3 space-y-3">
              <div className="rounded-xl border border-[#e5e7eb] bg-white p-4">
                <div className="flex items-center gap-2 mb-2">
                  <h4 className="text-sm font-semibold text-[#111827]">{risk.name}</h4>
                  <span className="rounded-full border border-[#da3633]/50 bg-[#da3633]/20 px-2 py-0.5 text-[10px] font-medium text-[#dc2626]">{risk.severity}</span>
                </div>
                <p className="text-xs text-[#6b7280] mb-3">{risk.summary}</p>
                <div className="rounded-lg border border-[#e5e7eb] bg-[#f9fafb] p-3">
                  <p className="text-[10px] font-medium uppercase tracking-wider text-[#9ca3af] mb-2">AI Analysis</p>
                  <pre className="text-xs text-[#6b7280] whitespace-pre-wrap font-mono leading-relaxed">{risk.aiAnalysis}</pre>
                </div>
              </div>
              <p className="text-sm font-medium text-[#111827] mt-3">
                Based on your domain expertise, what additional context can you provide about this risk?
              </p>
              <p className="text-xs text-[#6b7280] mt-1">For example: supplier diversification plans, timeline to impact, or controls already in place.</p>
            </div>
          ),
        }];
      });
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {[
                ...
                <button
                  ...
                  className="rounded-lg border border-[#e5e7eb] bg-[#f3f4f6] px-4 py-2 text-xs font-medium text-[#111827] hover:border-[#3b82f6]/50 hover:bg-[#3b82f6]/10 transition-colors"
                >
                  {opt.label}
                </button>
              ))}
        ...
        component: (
          <div className="mt-3 space-y-3">
            <div className="rounded-xl border border-[#3b82f6]/30 bg-[#3b82f6]/5 p-4">
              <p className="text-xs text-[#111827]">{risk.disclosureRec}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className="rounded-lg border border-[#e5e7eb] bg-[#f3f4f6] px-4 py-2 text-xs font-medium text-[#6b7280] hover:border-[#da3633]/50 hover:bg-[#da3633]/10 hover:text-[#ff7b72] transition-colors"
              >
                Request changes
              </button>
            </div>
      ...
      setMessages(prev => [...prev, {
        ...
        component: (
          <div className="mt-3 rounded-xl border border-[#3fb950]/30 bg-[#3fb950]/5 p-4">
            ...
            <p className="text-xs text-[#6b7280] mb-2">Thank you for your contribution. Your responses have been recorded and the GC will be notified.</p>
            <p className="text-xs text-[#6b7280] mb-2">Reference ID: INV-{riskId.replace("risk-", "").toUpperCase()}-2026</p>
            <p className="text-xs text-[#9ca3af]">You may close this window when finished.</p>
          </div>
        ),
      }]);
      setStep("complete");
        ...
        component: (
          <div className="mt-3">
            <textarea
              ...
              className="w-full rounded-lg border border-[#e5e7eb] bg-[#f9fafb] px-3 py-2 text-xs text-[#111827] placeholder-[#9ca3af] focus:border-[#3b82f6] focus:outline-none resize-none"
              rows={3}
            />
            <button
              onClick={() => {
              ...
              className="mt-2 rounded-lg bg-[#3b82f6] px-4 py-2 text-xs font-medium text-white hover:bg-[#79c0ff]"
            >
              Submit feedback
            </button>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#1a1d23] flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-10">
        <div className="w-full max-w-2xl rounded-xl border border-[#e5e7eb] bg-white shadow-2xl shadow-black/40 overflow-hidden flex flex-col" style={{ minHeight: "560px", maxHeight: "85vh" }}>
          ...
          <div className="border-b border-[#e5e7eb] bg-[#f9fafb] px-4 py-3 flex items-center justify-center flex-shrink-0">
        ...
        <div className="px-4 py-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs font-medium uppercase tracking-wider text-[#9ca3af]">Risk Investigation</span>
          </div>
          ...
          <div className="space-y-6">
                  ...
                  <div
                    className={cn(
                      ...
                      msg.role === "user"
                        ...
                        : "border border-[#e5e7eb] bg-[#f9fafb] rounded-bl-md"
                    )}
                  >
                    <p className="text-sm text-[#111827] whitespace-pre-wrap">{msg.content}</p>
                    {msg.component}
                    <p className="mt-2 text-[10px] text-[#9ca3af]">{msg.timestamp}</p>
                  </div>
                </div>
              </div>
            ))}
            {isLoading && (
              <div className="flex gap-3">
                ...
                <div className="rounded-2xl rounded-bl-md border border-[#e5e7eb] bg-[#f9fafb] px-4 py-3">
        ...
        <div className="border-t border-[#e5e7eb] bg-white p-4 flex-shrink-0">
          <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    input.trim() && !isLoading ? "bg-[#3b82f6] text-white hover:bg-[#79c0ff]" : "bg-[#f3f4f6] text-[#d1d5db]"
                  )}
                >
            ...
            <p className="mt-2 text-[10px] text-[#d1d5db]">
        ...
        <StakeholderFooter label="Switch to Chief Risk Officer to advance the workflow">
          <PrototypeControlButton
            onClick={() => {
              ...
              router.push(`/light/superhero/cro-review?risk=${riskId}&owner=${ownerId}`);
            }}
          >
