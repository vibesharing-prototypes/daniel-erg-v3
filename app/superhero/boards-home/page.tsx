...
/* ------------------------------------------------------------------ */
/*  Scan Sequence (V3 styled, inline in side panel)                    */
/* ------------------------------------------------------------------ */
...
function SidePanelScan({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div className="flex-1 flex flex-col px-5 py-6">
      <div className="flex-1 flex flex-col justify-center">
        {phase === "init" && (
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-[12px] text-slate-400 dark:text-zinc-500">Initializing GRC agents\u2026</p>
          </div>
        )}
        ...
        {phase !== "init" && (
          <div className="space-y-2">
                  ...
                  <div className="flex h-6 w-6 items-center justify-center rounded-lg flex-shrink-0">
                    {done ? (
                      <div className="flex h-6 w-6 items-center justify-center rounded-lg bg-emerald-100 dark:bg-emerald-900/40">
                        <svg className="w-3 h-3 text-emerald-600 dark:text-emerald-400" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 6l3 3 5-5"/></svg>
                      </div>
                    ) : (
                      <div className="w-4 h-4 border-2 border-slate-300 dark:border-zinc-600 border-t-transparent rounded-full animate-spin" />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-semibold text-slate-700 dark:text-zinc-200">{agent.label}</p>
                    <p className="text-[10px] text-slate-400 dark:text-zinc-500 truncate">{agent.detail}</p>
                  </div>
                </div>
              );
            })}
            ...
            {phase === "analyzing" && (
              <div className="flex items-center gap-3 rounded-xl border border-violet-200 dark:border-violet-900/40 bg-violet-50/50 dark:bg-violet-950/10 px-3 py-2.5 mt-2">
                <div className="w-4 h-4 border-2 border-violet-400 dark:border-violet-500 border-t-transparent rounded-full animate-spin flex-shrink-0" />
                <p className="text-[11px] font-semibold text-violet-600 dark:text-violet-400">Cross-referencing board materials\u2026</p>
              </div>
            )}

            {(phase === "detected" || phase === "ready") && anomalyCount > 0 && (
              <div className="rounded-xl border border-red-200 dark:border-red-900/40 bg-red-50/60 dark:bg-red-950/10 px-3 py-3 mt-2">
                <div className="flex items-center gap-2.5">
                  <div className="flex h-6 w-6 items-center justify-center rounded-full bg-red-100 dark:bg-red-900/40 flex-shrink-0">
                    <svg className="w-3 h-3 text-red-600 dark:text-red-400" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M8 1.5L1 14h14L8 1.5Z" /><path d="M8 6v3.5" /><circle cx="8" cy="11.5" r="0.5" fill="currentColor" />
                    </svg>
                  </div>
                  <div>
                    <p className="text-[12px] font-semibold text-red-700 dark:text-red-400">{anomalyCount} risk{anomalyCount !== 1 ? "s" : ""} detected</p>
                    <p className="text-[10px] text-red-500/70 dark:text-red-400/50">Not in current filings</p>
                  </div>
                </div>
              </div>
            )}
      ...
      <button onClick={onComplete} className="self-center text-[10px] text-slate-400 dark:text-zinc-500 hover:text-slate-600 dark:hover:text-zinc-300 transition-colors mt-4">
        Skip
      </button>
    </div>
  );
...
export default function BoardsHomePage() {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-[#f0f0f1] dark:bg-zinc-950">
      <ProtoPanel
        description="Boards Home \u2014 GRC Command Center side panel"
        stateToggle={false}
      />
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
                  ...
                  <div className="space-y-4">
                    {NEWS_ARTICLES.map((article, i) => (
                      <div key={i} className="pb-4 border-b border-[#f3f4f6] dark:border-zinc-800 last:border-0 last:pb-0">
                        ...
                        <div className="flex items-center gap-2 text-xs text-[#6b7280] dark:text-zinc-500 mb-1.5">
        ...
        {/* ------------------------------------------------------------------ */}
        {/*  GRC Command Center Side Panel — V3 styled                          */}
        {/* ------------------------------------------------------------------ */}
        <aside className="w-[340px] flex-shrink-0 border-l border-black/[0.09] dark:border-zinc-700 bg-[#f7f7f8] dark:bg-zinc-900 flex flex-col overflow-y-auto">

          {/* Header — matches Boards top bar h-12 */}
          <div className="h-12 flex items-center justify-between px-5 border-b border-black/[0.09] dark:border-zinc-700 flex-shrink-0 bg-white dark:bg-zinc-900">
            <div className="flex items-center gap-2.5">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg border border-black/[0.09] dark:border-zinc-700 bg-slate-50 dark:bg-zinc-800 p-1">
                <DiligentLogo className="h-4 w-4" />
              </div>
              <h2 className="text-[14px] font-semibold text-slate-800 dark:text-zinc-100">GRC Command Center</h2>
            </div>
            <Link href="/gc-commandcenter?direct=1" className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 dark:text-zinc-500 hover:text-slate-700 dark:hover:text-zinc-200 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors" title="Open full screen">
          ...
          {!scanComplete ? (
            <SidePanelScan onComplete={handleScanComplete} />
          ) : (
            <div className={cn("flex-1 flex flex-col transition-opacity duration-700", contentVisible ? "opacity-100" : "opacity-0")}>
              <div className="flex-1 space-y-4 pt-4">
                ...
                <div className="mx-4 rounded-[20px] border border-red-200 dark:border-red-900/40 bg-white dark:bg-zinc-900 overflow-hidden">
                  {/* Red glow */}
                  <div className="absolute top-0 left-0 right-0 h-16 pointer-events-none" style={{ background: "radial-gradient(ellipse 80% 100% at 50% 0%, rgba(239,68,68,0.07) 0%, transparent 100%)" }} />
                  <div className="relative p-4 space-y-3">
                    <div className="flex items-start gap-2.5">
                      <div className="mt-0.5 flex h-6 w-6 items-center justify-center rounded-full bg-red-100 dark:bg-red-900/40 flex-shrink-0">
                        <svg className="w-3 h-3 text-red-600 dark:text-red-400" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M8 1.5L1 14h14L8 1.5Z" /><path d="M8 6v3.5" /><circle cx="8" cy="11.5" r="0.5" fill="currentColor" />
                        </svg>
                      </div>
                      <div>
                        <p className="text-[13px] font-semibold text-red-700 dark:text-red-400">Agents Detected Emerging Risks</p>
                        <p className="text-[11px] text-red-500/70 dark:text-red-400/50 mt-0.5">3 risks require disclosure review</p>
                      </div>
                    </div>

                    <p className="text-[12px] text-slate-500 dark:text-zinc-400 leading-relaxed">
                      Monitoring agents detected emerging risks not adequately disclosed in current SEC filings or Board materials. Review before the Feb 28 Board meeting.
                    </p>
                    ...
                    <div className="space-y-2">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="inline-flex items-center rounded-full bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 px-2 py-0.5 text-[10px] font-semibold text-red-700 dark:text-red-400">1 Critical</span>
                        </div>
                        <div className="group relative inline-flex items-center gap-1.5 pl-1 cursor-help">
                          ...
                          <p className="text-[12px] text-slate-600 dark:text-zinc-300 leading-relaxed">Taiwan Strait Geopolitical Tensions</p>
                          <div className="absolute left-0 top-full mt-2 w-64 rounded-2xl border border-black/[0.09] dark:border-zinc-700 bg-white dark:bg-zinc-800 p-3 text-[11px] text-slate-500 dark:text-zinc-400 leading-relaxed shadow-[0_8px_28px_rgba(0,0,0,0.12)] opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity z-20">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="inline-flex items-center rounded-full bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 px-2 py-0.5 text-[10px] font-semibold text-amber-700 dark:text-amber-400">2 High</span>
                        </div>
                        <div className="space-y-1 pl-1">
                          <div className="group relative inline-flex items-center gap-1.5 cursor-help">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0 text-slate-300 dark:text-zinc-600 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors"><circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" /></svg>
                            <p className="text-[12px] text-slate-600 dark:text-zinc-300 leading-relaxed">Critical Vendor Cybersecurity Breach</p>
                            <div className="absolute left-0 top-full mt-2 w-64 rounded-2xl border border-black/[0.09] dark:border-zinc-700 bg-white dark:bg-zinc-800 p-3 text-[11px] text-slate-500 dark:text-zinc-400 leading-relaxed shadow-[0_8px_28px_rgba(0,0,0,0.12)] opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity z-20">
                          ...
                          <div className="group relative inline-flex items-center gap-1.5 cursor-help">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0 text-slate-300 dark:text-zinc-600 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors"><circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" /></svg>
                            <p className="text-[12px] text-slate-600 dark:text-zinc-300 leading-relaxed">EU Digital Markets Act Enforcement</p>
                            <div className="absolute left-0 top-full mt-2 w-64 rounded-2xl border border-black/[0.09] dark:border-zinc-700 bg-white dark:bg-zinc-800 p-3 text-[11px] text-slate-500 dark:text-zinc-400 leading-relaxed shadow-[0_8px_28px_rgba(0,0,0,0.12)] opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity z-20">
                    ...
                    <div className="border-t border-black/[0.05] dark:border-zinc-800 pt-3">
                      ...
                      <div className="space-y-1.5 text-[12px] text-slate-500 dark:text-zinc-400 leading-relaxed">
                ...
                {/* Divider */}
                <div className="h-px mx-4 bg-gradient-to-r from-transparent via-slate-200 dark:via-zinc-700 to-transparent" />

                {/* Other things on your plate */}
                <div className="px-4">
                  <h3 className="text-[13px] font-semibold text-slate-800 dark:text-zinc-100 mb-3">Other things on your plate</h3>
                  <div className="space-y-2">
                    {[
                      ...
                      <button key={item.label} className="flex items-center gap-2.5 w-full rounded-xl border border-black/[0.09] dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3.5 py-2.5 text-left hover:bg-slate-50 dark:hover:bg-zinc-700 hover:border-slate-200 dark:hover:border-zinc-600 transition-all group">
                        ...
                        <span className="text-[12px] text-slate-600 dark:text-zinc-300">{item.label}</span>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="border-t border-black/[0.05] dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-800/30 p-4 flex-shrink-0">
                <div className="relative">
                  <input
                    ...
                    className="w-full rounded-full border border-black/[0.09] dark:border-zinc-700 bg-white dark:bg-zinc-900 pl-4 pr-10 py-2.5 text-[12px] text-slate-700 dark:text-zinc-200 placeholder-slate-400 dark:placeholder-zinc-500 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                  <button
