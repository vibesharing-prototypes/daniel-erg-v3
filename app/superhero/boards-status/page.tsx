    ...
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="border-b-2 border-[#0ea5e9]/40 bg-[#e0f2fe] flex-shrink-0">
        <div className="border-b border-[#0ea5e9]/30 bg-[#bae6fd] px-4 py-2 flex items-center justify-between">
          <Link href="/" className="text-[10px] font-medium uppercase tracking-widest text-[#0369a1] hover:text-[#075985] transition-colors">Diligent Prototype — For illustrative and alignment purposes</Link>
          <button onClick={() => setDemoPanelOpen(!demoPanelOpen)} className="flex items-center gap-1 rounded px-2 py-0.5 text-[10px] font-medium text-[#0369a1] hover:bg-[#7dd3fc]/30 transition-colors">
              ...
              <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-3 flex-wrap gap-2">
                <div className="flex items-center gap-4">
                  <span className="rounded-full border-2 border-[#0c4a6e] bg-[#7dd3fc]/30 px-3 py-1 text-xs font-semibold text-[#0c4a6e]">
                    Viewing as: General Counsel — risks assigned, owners notified
                  </span>
                </div>
                <div className="flex items-center gap-1 rounded-xl border border-[#0ea5e9]/40 bg-white p-1">
                  <button onClick={() => router.push("/superhero/boards-home")} className="rounded-lg px-3 py-1.5 text-xs font-medium transition text-[#0369a1] hover:bg-[#bae6fd]">← Boards Home</button>
                  <button className="rounded-lg px-3 py-1.5 text-xs font-medium transition bg-[#0ea5e9] text-white">Step 3: Risks Underway</button>
                </div>
              </div>
            </div>
      ...
      <header className="h-12 bg-white border-b border-[#e2e5e9] flex items-center px-4 flex-shrink-0 z-10">
        ...
        <div className="flex items-center gap-1">
          {[<IconBell key="bell" />, <IconMessageCircle key="msg" />, <IconHelp key="help" />, <IconSettings key="set" />].map((icon, i) => (
            <button key={i} className="flex h-9 w-9 items-center justify-center rounded-lg text-[#6b7280] hover:bg-[#f3f4f6] transition-colors">{icon}</button>
          ))}
        </div>
      </header>
        ...
        <aside className="w-[210px] flex-shrink-0 bg-white border-r border-[#e2e5e9] flex flex-col self-stretch">
          ...
          <nav className="flex-1 py-1 px-2">
            {NAV_ITEMS.map((item, i) => {
              ...
              return (
                <button key={item.label} className={cn("flex items-center gap-3 w-full rounded-lg px-3 py-2 text-sm transition-colors text-left", item.active ? "bg-[#eff6ff] text-[#2563eb] font-medium" : "text-[#374151] hover:bg-[#f3f4f6]")}>
          ...
          <div className="px-4 py-3 border-t border-[#e2e5e9] flex items-center justify-between">
            <div className="flex items-center gap-1.5"><DiligentLogo className="h-4 w-4" /><span className="text-xs font-semibold text-[#111827]">Diligent</span></div>
                ...
                {BOOKS.map((book) => (
                  <div key={book.id} className="rounded-xl border border-[#e2e5e9] bg-white p-5">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {book.badges.map((badge) => (<span key={badge} className={cn("rounded px-2 py-0.5 text-xs font-medium", badge === "Approval required" ? "bg-[#fef2f2] text-[#dc2626]" : "bg-[#eff6ff] text-[#2563eb]")}>{badge}</span>))}
                    </div>
                    ...
                    <div className="flex items-center gap-4 mt-4 pt-3 border-t border-[#f3f4f6]">
                      ...
                      <button className="flex items-center gap-1.5 text-xs font-medium text-[#6b7280] hover:text-[#374151] transition-colors">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M4 6h16M4 12h16M4 18h10" /></svg>Summary
                      </button>
                      <button className="flex items-center gap-1.5 text-xs font-medium text-[#6b7280] hover:text-[#374151] transition-colors">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M4 6h16M4 10h16M4 14h10M4 18h6" /></svg>Insights
                      </button>
              ...
              <div className="space-y-4">
                {NEWS_ARTICLES.map((article, i) => (
                  <div key={i} className="pb-4 border-b border-[#f3f4f6] last:border-0 last:pb-0">
                    <a href="#" className="text-sm font-medium text-[#2563eb] hover:underline leading-snug block mb-1">{article.title}</a>
                    <div className="flex items-center gap-2 text-xs text-[#6b7280] mb-1.5"><span>{article.source}</span><span>{article.date}</span></div>
      ...
      {/* ------------------------------------------------------------------ */}
      {/*  GRC Command Center — Risks Underway                                */}
      {/* ------------------------------------------------------------------ */}
      <aside className="w-[340px] flex-shrink-0 border-l border-[#1e293b] bg-[#0f172a] flex flex-col overflow-y-auto">
        <div className="flex items-center justify-between px-4 pt-4 pb-3 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#1e293b] border border-[#334155] p-1"><DiligentLogo className="h-4 w-4" /></div>
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex-1 px-4 pt-4 space-y-4">
            ...
            {/* Status banner */}
            <div className="rounded-lg border border-[#065f46]/40 bg-[#052e16]/30 p-3">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-xs font-semibold text-[#34d399]">Risk owners notified</p>
              </div>
              <p className="text-[11px] text-[#94a3b8] leading-relaxed">
                All 3 owners have been notified and asked to respond within 48 hours. You&apos;ll receive updates as they respond.
              </p>
            </div>
            ...
            <div className="space-y-2.5">
              {ASSIGNED_RISKS.map((risk) => (
                <div key={risk.id} className="rounded-lg border border-[#1e293b] bg-[#0f172a] p-3">
                  {/* Risk name — compact */}
                  <div className="flex items-center gap-2 mb-2">
                    <span className={cn(
                      "inline-flex items-center rounded-full px-1.5 py-px text-[8px] font-bold uppercase flex-shrink-0",
                      risk.severity === "Critical" ? "bg-[#dc2626]/20 text-[#fca5a5]" : "bg-[#f97316]/20 text-[#fdba74]"
                    )}>
                      {risk.severity}
                    </span>
                    <p className="text-[11px] font-medium text-[#cbd5e1] leading-snug truncate">{risk.name}</p>
                  </div>

                  {/* Owner + status */}
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <img src={risk.avatarUrl} alt={risk.owner} className="h-5 w-5 rounded-full object-cover flex-shrink-0" />
                      <div className="min-w-0">
                        <p className="text-[10px] text-[#94a3b8] truncate">{risk.owner} · {risk.title}</p>
                      </div>
                    </div>
                    <button className="flex-shrink-0 text-[10px] text-[#60a5fa] hover:text-[#93c5fd] transition-colors">
                      Follow up
                    </button>
                  </div>

                  {/* Timestamp */}
                  <div className="flex items-center gap-1.5 mt-2 pt-2 border-t border-[#1e293b]">
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#475569" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>
                    <span className="text-[9px] text-[#475569]">Notified today at {risk.notifiedAt} · {risk.status}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="border-t border-[#334155] bg-[#1e293b] p-4 flex-shrink-0">
            <div className="relative">
              <input type="text" placeholder="Ask about risk status..." className="w-full rounded-full border border-[#475569] bg-[#0f172a] pl-4 pr-10 py-2.5 text-xs text-[#e2e8f0] placeholder-[#64748b] focus:border-[#3b82f6] focus:outline-none focus:ring-1 focus:ring-[#3b82f6]" />
              <button className="absolute right-1.5 top-1/2 -translate-y-1/2 flex h-7 w-7 items-center justify-center rounded-full bg-[#3b82f6] text-white hover:bg-[#2563eb] transition-colors">
