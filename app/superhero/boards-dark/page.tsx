...
export default function BoardsDarkPage() {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="border-b-2 border-[#1e40af]/40 bg-[#0c1929] flex-shrink-0">
        <div className="border-b border-[#1e40af]/30 bg-[#0f2340] px-4 py-2 flex items-center justify-between">
          <p className="text-[10px] font-medium uppercase tracking-widest text-[#93c5fd]">
            Demo controls — not part of prototype
          </p>
          <button
            onClick={() => setDemoPanelOpen(!demoPanelOpen)}
            className="flex items-center gap-1 rounded px-2 py-0.5 text-[10px] font-medium text-[#93c5fd] hover:bg-[#1e3a5f]/30 transition-colors"
          >
        ...
        {demoPanelOpen && (
          <>
            <div className="w-full border-b border-[#1e40af]/20 bg-[#0c1929]">
              <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-3 flex-wrap gap-2">
                <div className="flex items-center gap-4">
                  <span className="rounded-full border-2 border-[#1e40af] bg-[#1e3a5f]/30 px-3 py-1 text-xs font-semibold text-[#93c5fd]">
                    Viewing as: Change-Averse General Counsel using Boards
                  </span>
                </div>
                <div className="flex items-center gap-1 rounded-xl border border-[#1e40af]/40 bg-[#161b22] p-1">
                  <button className="rounded-lg px-3 py-1.5 text-xs font-medium transition bg-[#1e40af] text-white">
                    Near-Term: Boards Entry
                  </button>
                  <button
                    onClick={() => router.push("/gc-commandcenter")}
                    className="rounded-lg px-3 py-1.5 text-xs font-medium transition text-[#93c5fd] hover:bg-[#0f2340]"
                  >
            ...
            <div className="w-full border-b border-[#1e40af]/20 bg-[#0c1929]">
              <div className="mx-auto max-w-7xl px-4 py-3">
                <div className="flex items-start gap-3">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#450a0a]">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-[#93c5fd]">
                      <span className="font-medium text-[#60a5fa]">Scenario:</span> The General Counsel opens the standard Boards system enhanced with the GRC Command Center panel and sees that monitoring agents have detected 
            ...
            <div className="border-t-2 border-[#1e40af] bg-[#0f172a] px-4 py-2">
              <p className="text-[10px] font-medium uppercase tracking-widest text-[#60a5fa]">
      ...
      {/* Top bar */}
      <header className="h-12 bg-[#161b22] border-b border-[#30363d] flex items-center px-4 flex-shrink-0 z-10">
        <div className="flex items-center gap-2.5 pl-1">
          <DiligentLogo className="h-6 w-6" />
          <span className="text-sm font-semibold text-[#c9d1d9]">Boards</span>
          <span className="text-xs text-[#6e7681]">(GC view)</span>
        </div>
        <div className="flex-1" />
        <div className="flex items-center gap-1">
          {[<IconBell key="bell" />, <IconMessageCircle key="msg" />, <IconHelp key="help" />, <IconSettings key="set" />].map((icon, i) => (
            <button key={i} className="flex h-9 w-9 items-center justify-center rounded-lg text-[#8b949e] hover:bg-[#21262d] transition-colors">
              {icon}
            </button>
          ))}
        </div>
      ...
      <div className="flex flex-1 overflow-y-auto">
        ...
        {/*  Left Sidebar                                                       */}
        {/* ------------------------------------------------------------------ */}
        <aside className="w-[210px] flex-shrink-0 bg-[#161b22] border-r border-[#30363d] flex flex-col self-stretch">
          <div className="px-4 py-4 flex items-center gap-2">
            <div className="flex h-7 w-7 items-center justify-center rounded bg-[#1d4ed8]">
            ...
            <span className="text-sm font-bold text-[#58a6ff]">Walmart</span>
            <span className="text-sm text-[#f59e0b]">✳</span>
          </div>
          <nav className="flex-1 py-1 px-2">
            {NAV_ITEMS.map((item, i) => {
              if (item.separator) return <div key={i} className="my-2 mx-3 border-t border-[#30363d]" />;
              return (
                <button
                  key={item.label}
                  className={cn(
                    ...
                    item.active
                      ? "bg-[#0c2d6b] text-[#58a6ff] font-medium"
                      : "text-[#c9d1d9] hover:bg-[#21262d]"
                  )}
                >
                  <span className={cn(item.active ? "text-[#58a6ff]" : "text-[#8b949e]")}>{item.icon}</span>
                  <span className="flex-1">{item.label}</span>
                  {item.external && <span className="text-[#58a6ff]"><IconExternalLink /></span>}
                </button>
              );
            })}
          </nav>
          <div className="px-4 py-3 border-t border-[#30363d] flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs font-semibold text-[#f0f6fc]">Diligent</span>
            </div>
            <span className="text-[10px] text-[#6e7681]">1.25…</span>
          </div>
        </aside>
          ...
          <div className="max-w-[780px] mx-auto px-6 py-6 space-y-5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-[#30363d] bg-[#161b22] pl-10 pr-4 py-2 text-sm text-[#c9d1d9] placeholder-[#484f58] focus:border-[#58a6ff] focus:outline-none focus:ring-1 focus:ring-[#58a6ff]"
              />
            </div>

            {/* Awaiting review */}
            <div className="rounded-xl border border-[#30363d] bg-[#161b22] px-4 py-3">
              <h2 className="text-sm font-semibold text-[#f0f6fc] mb-1">Awaiting review</h2>
              <div>
                {[
                  ...
                  <button key={item.label} className="flex items-center justify-between w-full rounded-lg px-2 py-2 hover:bg-[#161b22] transition-colors group">
                    ...
                    <span className="text-[#484f58] group-hover:text-[#6e7681]">
                      <IconChevronDown />
                    </span>
                  </button>
                ))}
            ...
            {/* Books and reports */}
            <div>
              <h2 className="text-base font-semibold text-[#f0f6fc] mb-3">Books and reports</h2>
              ...
              <div className="space-y-4">
                {BOOKS.map((book) => (
                  <div key={book.id} className="rounded-xl border border-[#30363d] bg-[#161b22] p-5">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-[#8b949e]"><IconFileText /></span>
                        ...
                        <span
                          ...
                          className={cn(
                            ...
                            badge === "Approval required"
                              ? "bg-[#3b1219] text-[#dc2626]"
                              : "bg-[#0c2d6b] text-[#58a6ff]"
                          )}
                        >
                          {badge}
                        </span>
                    ...
                    <h3 className="text-sm font-semibold text-[#f0f6fc] mb-1">{book.title}</h3>
                    <p className="text-xs text-[#8b949e]">
                      {book.meeting}
                      {(book as any).extra && <span className="ml-1.5 inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#30363d] text-[10px] font-medium text-[#8b949e]">{(book as any).extra}</span>}
                    </p>
                    <div className="flex items-center gap-4 mt-4 pt-3 border-t border-[#21262d]">
                      <button className="rounded-lg border border-[#30363d] bg-[#161b22] px-3 py-1.5 text-xs font-medium text-[#c9d1d9] hover:bg-[#161b22] transition-colors">
                        Open book
                      </button>
                      <button className="flex items-center gap-1.5 text-xs font-medium text-[#8b949e] hover:text-[#c9d1d9] transition-colors">
                      ...
                      <button className="ml-auto text-[#6e7681] hover:text-[#8b949e]">
                        <IconMoreVertical />
                      </button>
                    </div>
                  </div>
        ...
        {/*  Right Sidebar                                                      */}
        {/* ------------------------------------------------------------------ */}
        <aside className="w-[320px] flex-shrink-0 border-l border-[#30363d] bg-[#161b22]">
          <div className="p-5 space-y-6">

            {/* News and Insights */}
            <div>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-[#f0f6fc]">News and Insights</h2>
                <button className="text-[#484f58] hover:text-[#6e7681]"><IconChevronDown /></button>
              </div>
              <div className="flex items-center gap-1.5 mb-4">
                <IconSparkles />
                <span className="text-xs font-medium text-[#c9d1d9]">Articles summarized by AI</span>
                <span className="rounded px-1.5 py-0.5 text-[10px] font-semibold text-[#a78bfa] bg-[#2e1065]">BETA</span>
              </div>

              <div className="space-y-4">
                {NEWS_ARTICLES.map((article, i) => (
                  <div key={i} className="pb-4 border-b border-[#21262d] last:border-0 last:pb-0">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <a href="#" className="text-sm font-medium text-[#58a6ff] hover:underline leading-snug">{article.title}</a>
                    </div>
                    <div className="flex items-center gap-2 text-xs text-[#8b949e] mb-1.5">
                      <span>{article.source}</span>
                      <span>{article.date}</span>
                    </div>
                    <p className="text-xs text-[#8b949e] leading-relaxed">{article.summary}</p>
                  </div>
                ))}
              </div>

              <button className="w-full mt-3 rounded-lg border border-[#30363d] bg-[#161b22] py-2 text-xs font-medium text-[#c9d1d9] hover:bg-[#161b22] transition-colors">
                Load more
              </button>
              <p className="text-[10px] text-[#6e7681] mt-2 text-center">
                AI-generated content may have inaccuracies.{" "}
                <button className="font-medium underline hover:text-[#8b949e]">Learn more</button>
              </p>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-[#f0f6fc]">Certifications and templates</h2>
                <button className="text-[#484f58] hover:text-[#6e7681]"><IconChevronDown /></button>
              </div>
              <div className="rounded-xl border border-[#30363d] bg-[#161b22] p-4 text-center">
                <div className="flex items-center justify-center gap-2 mb-3">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="h-10 w-10 rounded-lg bg-[#30363d] flex items-center justify-center">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6e7681" strokeWidth="1.5"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>
                    </div>
                  ))}
                </div>
                ...
                <button className="w-full rounded-lg border border-[#30363d] bg-[#161b22] py-2 text-xs font-medium text-[#c9d1d9] hover:bg-[#161b22] transition-colors">
          ...
          <div className="border-t border-[#334155] bg-[#1e293b] p-4">
            <div className="relative">
              <input
                ...
                className="w-full rounded-full border border-[#475569] bg-[#0f172a] pl-4 pr-10 py-2.5 text-xs text-[#e2e8f0] placeholder-[#64748b] focus:border-[#58a6ff] focus:outline-none focus:ring-1 focus:ring-[#58a6ff]"
              />
              <button
                onClick={() => router.push("/superhero/coordinator")}
                className="absolute right-1.5 top-1/2 -translate-y-1/2 flex h-7 w-7 items-center justify-center rounded-full bg-[#3b82f6] text-white hover:bg-[#1d4ed8] transition-colors"
              >
